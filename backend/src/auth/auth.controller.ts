...
export class AuthController {
    ...
    googleRedirect(@Res() res: Response, @Req() req){
        ...
        if(user.first_time)
        {
            ...
            res.cookie('token', token, { httpOnly: true, maxAge: 600000000000 });
            this.gameState.createStateGame(user.id);
            return res.redirect("http://10.12.2.12:3000/dashboard/settings");
        }
        ...
        if(user.tfa_enabled)
        {
            const token = this.jwtService.sign(payload);
            res.cookie('token', token, { httpOnly: true, maxAge: 600000000000 });
            return res.redirect("http://10.12.2.12:3000/signIn/verify-two-factor");
        }

        const token = this.jwtService.sign(payload)
        res.cookie('token', token, { httpOnly: true, maxAge: 600000000000 });
        return res.redirect("http://10.12.2.12:3000/dashboard")
    }
