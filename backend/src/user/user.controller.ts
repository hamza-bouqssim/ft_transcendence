...
export class UserController {
	...
	async displayedName(@Body() request: { newDisplayName: string }, @Req() req) {
		try {
			...
			const updated = this.userService.changeDisplayedName(
				user.email,
	...
	async changeUserName(@Body() request: { newUserName: string }, @Req() req) {
		try {
			...
			const updated = this.userService.changeUserName(
				user.email,
				req.newUserName,
			);
			return updated;
		...
		FileInterceptor('file', {
			...
			fileFilter: (req, file, _callback) => {
				if (file.mimetype === 'image/jpeg' || file.mimetype === 'image/png') {
					...
					_callback(
						new UnauthorizedException('Only JPEG and PNG files are allowed'),
						false,
					); // refuse somthing else like .pdf ...etc
				}
			},
			limits: {
	...
	async changeAvatar(@Req() req, @UploadedFile() file: Express.Multer.File) {
		try {
			...
			const updatedAvatar = this.userService._changeAvatar(
				user.email,
				imagePath,
			);
	...
	async searchUsers(@Body() request: { displayName: string }) {
		const test = this.userService.findByDisplayNameSearching(
			request.displayName,
		);
		return test;
	}
