	...
	async createConversations(user: User, params: CreateConversationParams) {
		...
		if (!recipient)
			throw new HttpException(
				'User not found so cannot create Conversation',
				HttpStatus.BAD_REQUEST,
			);

		// const userDb = await this.userService.checkIfParticipent(user.id);
		const Participent = await this.participentService.findParticipent(
			params,
			user,
		);
		if (!Participent) {
			const newParticipant = this.participentService.CreateParticipent(
				params,
				user,
			);
