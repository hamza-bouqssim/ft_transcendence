	...
	implements OnGatewayConnection, OnGatewayDisconnect
{
	...
	async handleConnection(socket: AuthenticatedSocket) {
		...
		console.log('user conncted with socket.io', userdb);
	...
	async handleMessage(
		...
		const chatroom = await this.roomsService.notificationRoomUpdate(
			messageRome.senderId,
			messageRome.chatRoomId,
		);
		chatroom.members.map((member) => {
			if (member.id !== messageRome.senderId)
				this.server.to(member.user_id.toString()).emit('setNotification', {
					id: chatroom.id,
					content: messageRome.content,
				});
		});
		this.server
	...
	handleTyping(client: Socket, { id, userId }) {
		this.server
			.to(id.toString())
			.emit('Typing', { status: true, userId: userId });
	}
	...
	handleLeaveTyoing(client: Socket, { id, userId }) {
		this.server
			.to(id.toString())
			.emit('Typing', { status: false, userId: userId });
	}
	...
	async searchRoom(client: AuthenticatedSocket, val: string) {
		...
		this.server
			.to(client.user.sub.toString())
			.emit('resualtRoom', { resualt: resualt });
	}
	...
	async cleanNotification(client: AuthenticatedSocket, roomId: string) {
		...
		this.server
			.to(client.user.sub.toString())
			.emit('setNotification', { id: roomId });
	}

	@OnEvent('order.created')
	async onNotificationCreate(data: any) {
		const userAdmin = data.members.find(
			(userAdmin) => userAdmin.Status === 'Owner',
		);
	...
	sendRequestToPLay(data: any) {
		...
		this.server
			.to(data.requestToPlay.recipient.id)
			.emit(`newRequestToPlay`, data);
		this.userService.createNotification(
			data.requestToPlay.Sender,
			data.requestToPlay.recipient,
			message,
			type,
			requestId,
		);
	...
	RefuserFriendRequestNotification(RefuseruserId: string) {
		this.server.emit(
			'RefuseNotification',
			`${RefuseruserId} refuse your request`,
		);
	}
	...
	createConversation(data: any) {
		this.server
			.to(data.conversation.sender.id)
			.to(data.conversation.recipient.id)
			.emit('createConversation', data);
	}

	@OnEvent('createConversationMessage.created')
	createConversationMessage(data: any) {
		this.server
			.to(data.chat.sender.id)
			.to(data.chat.recipient.id)
			.emit('createConversationMessage', data);
	}
	...
	deleteConversation(payload: { chatParticipent: any; userId: string }) {
		this.server
			.to(payload.userId)
			.emit('deleteConversation', payload.chatParticipent);
	}
	...
	async handleInvitegame(data: any) {
		this.server
			.to(data.requestToPlay.recipientId)
			.emit('newRequestToPlay', data);
	}
	...
	async handleAcceptPLayNotification(data: any) {
		...
		this.server
			.to(data.req_play.senderId)
			.emit('AcceptPLayNotification', { accept: true });
		this.server
			.to(data.req_play.recipientId)
			.emit('AcceptPLayNotification', { accept: true });
	}
	...
	async handleMessageCreateEvent(socket: AuthenticatedSocket, payload: any) {
		...
		this.server
			.to(messages.participents.recipientId)
			.to(messages.participents.senderId)
			.to(messages.participentsId.toString())
			...
			if (sessionNumber > 0) {
				...
				if (userId) {
					...
					if (existingUser) {
						...
						console.error(
							`User with ID ${userId} not found. Unable to update status.`,
						);
					}
				}
			}
