...
const Team = () => {
  ...
  const mappedNames: JSX.Element[] = teams.map(
    (e: string, i: number): JSX.Element => (
      <Image
        className="select-none cursor-pointer rounded-[50%] border-[2px] border-[var(--blue-color)] transition duration-300 ease-linear hover:scale-90 md:w-28 lg:w-36 xl:w-48 2xl:w-56"
        key={i}
        src={e}
        width={100}
        height={100}
