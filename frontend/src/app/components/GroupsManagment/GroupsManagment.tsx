...
const GroupsManagement = () => {
  ...
  if (status.get == "loading" || status.get == "idle") {
    return (
      <div role="status">
    <svg aria-hidden="true" className="w-8  h-8  mx-auto mt-20  fill-[#5B8CD3]  text-gray-200 animate-spin dark:text-gray-600 " viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
		<div className="no-scrollbar  h-[calc(100%-74px)] overflow-auto pb-2 pt-5  text-black">
			{sortedConversations &&
				sortedConversations?.map((data: ConversationTypes) => (
					<div key={data.id}>
