...
const OnlineGame = forwardRef((props: any, ref: any) => {
	...
	return (
		<div className="absolute left-[50%] top-[50%] flex h-[642px] w-[96%] -translate-x-[50%] -translate-y-[50%] flex-col items-center justify-evenly md:h-[900px] md:gap-3 md:px-4 xl:h-[700px] xl:max-w-[1280px] xl:flex-row-reverse xl:px-10 min-[1750px]:h-[900px] min-[1750px]:max-w-[1600px]">
			<PlayerScore
				flag="top"
				userName={props.opponentPlayer?.username}
				displayName={props.opponentPlayer?.display_name}
				score={props.score.playerOne}
				color={"#4FD6FF"}
				profileImage={props.opponentPlayer?.avatar_url}
				startGame={props.startGame}
			/>
			<div
				className={`${
					props.rotateRef.current ? "rotate-180" : ""
