...
const Dashboard = ({ params }: { params: { id: string } }) => {
  const { friendsBlock, blocked, fstatus, ferror } = useSelector(
    (state: any) => state.friendsBlock
  );
  ...
  const { requests, statusReq, errorReq } = useSelector(
  ...
  const handleClickAcceptRequest = async () => {
    ...
    try {
      if (matchingRequest) {
        ...
        const res = await dispatch(
          fetchAcceptFriendRequestThunk(matchingRequest.id)
        );
      ...
      <div className="container">
        <div className="row">
          <div className="col-1">
                  ...
                  <>
                    {buttonType === "pendingRequest" ? (
                      ...
                      <button
                        ...
                        className="h-12 rounded-2xl bg-[--purple-color] px-4 shadow-xl ease-in-out hover:scale-105 hover:bg-[--purple-hover] hover:duration-300"
                      >
                        Respond to Request
                      </button>
                    ) : buttonType === "friends" ? (
                      <button
                        className="h-12 rounded-2xl bg-[--pink-color] px-4 shadow-xl ease-in-out hover:scale-105 hover:bg-[--gray-hover] hover:duration-300 hover:bg-[--pink-hover]"
                        disabled
                      >
                        Friends
                      </button>
            ...
            {!blocked.some(
              (elem: BloqueList) =>
                (elem.userOne.id === Userdata?.id &&
                  elem.userTwo.id === params.id) ||
                (elem.userOne.id === params.id &&
                  elem.userTwo.id === Userdata?.id)
            ) ? (
              <div>
                <div className="boxes">
