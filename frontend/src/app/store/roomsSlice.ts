	...
	extraReducers: (builder: any) => {
		builder
			...
			.addCase(
				...
				(state: any, action: PayloadAction<Room[]>) => {
					state.status = "succeeded";
					state.rooms = (action.payload as any).data;
			...
			.addCase(createRooms.fulfilled, (state: any, action: any) => {
				const newRoom = action.payload as any;
				state.rooms.push(newRoom);
			})
			.addCase(
				updateRooms.fulfilled,
				(state: any, action: PayloadAction<Room>) => {
					const updateData = (action.payload as any).data;
					console.log(updateData);
					const index = state.rooms.findIndex(
						(room: Room) => room.id === updateData.id,
					);
					if (index !== -1) {
						state.rooms[index] = (action.payload as any).data;
					}
				},
			)
			.addCase(
				deleteRooms.fulfilled,
				(state: any, action: PayloadAction<Room>) => {
					const deletedRoom = (action.payload as any).data;
					const index = state.rooms.findIndex(
						(room: Room) => room.id === deletedRoom.id,
					);
					if (index !== -1) {
